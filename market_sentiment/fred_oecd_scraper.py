"""
FRED OECD Consumer Confidence Scraper

Scrapes OECD consumer confidence series from FRED API:
1. USACSCICP02STSAM - Composite Consumer Confidence for United States
2. CSCICP03USM665S - Consumer Confidence Amplitude Adjusted
...
class FREDOECDScraper:
    """Scraper for FRED OECD consumer confidence series"""
    ...
    SERIES = {
        'composite_confidence': {
            ...
            'field_name': 'oecd_composite_confidence'
        },
        'amplitude_adjusted': {
            'id': 'CSCICP03USM665S',
            'name': 'OECD Consumer Confidence Amplitude Adjusted',
            'field_name': 'oecd_amplitude_adjusted'
        }
    }
    ...
    def fetch_all_series(self) -> bool:
        ...
        """
        logger.info("=" * 80)
        logger.info("Fetching OECD consumer confidence data from FRED")
        logger.info("=" * 80)
    ...
    def save_data(self, combined_df: pd.DataFrame, filename: str = 'fred_oecd_confidence.csv') -> str:
        """
        ...
        # Save metadata
        metadata_path = os.path.join(self.output_dir, 'fred_oecd_metadata.json')
        import json
    ...
    def validate_data(self, combined_df: pd.DataFrame) -> Dict:
        ...
        results = {
            ...
            'date_range': (
                str(combined_df['date'].min().date()),
                str(combined_df['date'].max().date())
            ),
            'series': {}
        }
        ...
        for key, series_info in self.SERIES.items():
            ...
            if field_name in combined_df.columns:
                ...
                series_results = {
                    ...
                    'latest_date': str(combined_df[series_data.notna()]['date'].iloc[-1].date()) if series_data.notna().any() else None
                }
                ...
                logger.info(f"  Latest: {series_results['latest']} ({series_results['latest_date']})")
    ...
    def run(self) -> bool:
        ...
        logger.info("\n" + "=" * 80)
        logger.info("FRED OECD CONSUMER CONFIDENCE SCRAPER")
        logger.info("=" * 80)
...
def main():
    """Main entry point"""
    # Create scraper
    scraper = FREDOECDScraper(output_dir='FRED/data')

    # Run scraping pipeline
    success = scraper.run()

    if success:
        logger.info("\n🎉 SUCCESS! FRED OECD consumer confidence data scraped! 🎉")
    else:
