"""
FRED Cleveland Fed Inflation Expectations Scraper

Scrapes Cleveland Fed inflation expectation series from FRED API:
...
class FREDClevelandScraper:
    """Scraper for FRED Cleveland Fed inflation expectations"""
    ...
    SERIES = {
        'expinf1yr': {
            ...
            'field_name': '1_year_expected_inflation'
        },
        'expinf10yr': {
            'id': 'EXPINF10YR',
            'name': '10-Year Expected Inflation',
            'field_name': '10_year_expected_inflation'
        }
    }
    ...
    def fetch_all_series(self) -> bool:
        ...
        """
        logger.info("=" * 80)
        logger.info("Fetching Cleveland Fed inflation expectations from FRED")
        logger.info("=" * 80)
    ...
    def save_data(self, combined_df: pd.DataFrame, filename: str = 'fred_cleveland_inflation.csv') -> str:
        """
        ...
        # Save metadata
        metadata_path = os.path.join(self.output_dir, 'fred_cleveland_metadata.json')
        import json
    ...
    def run(self) -> bool:
        ...
        logger.info("\n" + "=" * 80)
        logger.info("FRED CLEVELAND FED INFLATION EXPECTATIONS SCRAPER")
        logger.info("=" * 80)
...
def main():
    """Main entry point"""
    # Create scraper
    scraper = FREDClevelandScraper(output_dir='FRED/data')

    # Run scraping pipeline
    success = scraper.run()

    if success:
        logger.info("\n🎉 SUCCESS! Cleveland Fed inflation expectations scraped! 🎉")
    else:
